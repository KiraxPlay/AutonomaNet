...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        async function checkLogin() {
            try {
                const res = await verifyTokenRequest();  // ⬅️ llama a /verify
                setUser(res.data);                       // ⬅️ guarda el usuario
                setIsAuthenticated(true);                // ⬅️ indica que está autenticado
            } catch (error) {
                setUser(null);
                setIsAuthenticated(false);
            } finally {
                setLoading(false);                       // ⬅️ desactiva el loader
            }
        }
        checkLogin();
    }, []);
